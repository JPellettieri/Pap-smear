...
PAP) Se recomienda que se realicen este estudio las mujeres a partir de los 25 a�os.
https://www.argentina.gob.ar/salud/cancer/tipos/cancer-de-cuello-de-utero-ccu

MAMO) Se recomienda que todas las mujeres de 50 a 69 a�os se realicen al menos una mamograf�a cada dos a�os
...
CMT_maker <- function(df, vivienda, b, c, d, e, f, g, h){
  ...
  NBI_1 <- df[[vivienda]] #bhcv01:Tipo de vivienda (1. Casa  2. Casilla 3. Depertamento  4. Pieza de inquilinato 5. Pieza en hotel o pensi�n 6. Local no construido para habitaci�n+B33 7. Otros
  NBI_1[NBI_1 == 1 | NBI_1 == 3] <- 0 # casa o departamento
  NBI_1[!(NBI_1 == 0)] <- 1 # todo el resto se considera carente
  
  
  NBI_2 <- df[[b]] #bhcv10 (ba�o):
...
sim <- simulateResiduals(fittedMod = m_comp, plot = T) # KS
...
plot(comp_e[[1]], comparisons = TRUE)
...
roc(control_pap ~ rango_edad + cobertura_salud + quintil_uc + Carencia + instr_agrupada + (1|cod_provincia), prueba1)
roc()
...
plot(ggpredict(m_comp, terms = ~rango_edad + cobertura_salud + instr_agrupada))
plot(ggpredict(m_comp, terms = ~rango_edad + quintil_uc + Carencia))
